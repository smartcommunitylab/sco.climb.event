...
var searchTableCtrl = searchTableApp.controller('userCtrl', function($scope, $http, $window, DataService) {
	...
	$scope.changeSchool = function() {
		...
		function (response) {
			...
			var urlChildList = $scope.contextApiUrl + "child/" + $scope.profile.ownerId
			+ "/" + $scope.selectedSchool.objectId;
			$http.get(urlChildList, {headers: {'X-ACCESS-TOKEN': $scope.profile.token}}).then(
				function (response) {
	...
	$scope.searchItem = function() {
		...
		var urlSearch = "api/check/" + $scope.profile.ownerId 
